...
3. 한 번 실행할 때 새 API 요청은 기본 최대 40건, 요청 사이 대기는 15초다. Actions Variables `GEMINI_MAX_REQUESTS`(1~400), `GEMINI_DELAY_MS`(5000~60000)로 조절할 수 있다. 실제 할당량을 보장하는 수치는 아니다. 5초 하한은 무료 티어 약 10 RPM 아래를 유지하며, 429가 나도 저장 후 멈추고 다음 실행이 이어간다. 한 회차 전체(약 300건)를 한 번에 끝내려면 `GEMINI_MAX_REQUESTS=400`, `GEMINI_DELAY_MS=6000`(약 30분). 새 모델을 처음 쓸 때는 `GEMINI_MAX_REQUESTS`를 2~3으로 낮춰 용량·응답을 먼저 확인한 뒤 되돌린다. 모델이 바뀌면 캐시 식별자가 바뀌어 앞선 판정은 재사용되지 않는다.
